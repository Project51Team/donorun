...
export class TransactionGroupComponent implements OnInit {
  ...
  loadAll(): void {
    ...
    this.transactionGroupService
      ...
      .subscribe({
        next: (res: HttpResponse<ITransactionGroup[]>) => {
          this.isLoading = false;
          this.paginateTransactionGroups(res.body, res.headers);
        },
        error: () => {
          this.isLoading = false;
        },
  ...
  trackId(_index: number, item: ITransactionGroup): string {
    return item.id!;
  }

  delete(transactionGroup: ITransactionGroup): void {
    const modalRef = this.modalService.open(TransactionGroupDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.transactionGroup = transactionGroup;
  ...
  protected paginateTransactionGroups(data: ITransactionGroup[] | null, headers: HttpHeaders): void {
